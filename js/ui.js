    ...
    historyList.innerHTML = history.map(item => {
        ...
        if (item.episodes && Array.isArray(item.episodes) && item.episodes.length > 0) {
            ...
            const syncStatus = item.lastSyncTime ?
                ...
                `<span class="text-yellow-400 text-xs" title="使用暫存數劇">⚠</span>`;
...
function deleteHistoryItem(encodedUrl) {
    try {
        ...
        // 显示成功提示
        showToast('已删除該紀錄, 'success');
    } catch (e) {
        console.error('删除历史记录项失败:', e);
        showToast('删除紀錄失敗', 'error');
    }
}
        ...
        if (historyItem && historyItem.vod_id && historyItem.sourceName) {
            ...
            try {
                ...
                if (videoDetails && videoDetails.episodes && videoDetails.episodes.length > 0) {
                    ...
                    showToast('未獲取到最新劇集訊息，使用暫存數據', 'warning');
                }
            } catch (fetchError) {
                // console.error(`获取 "${title}" 最新剧集列表失败:`, fetchError, "将使用已存储的剧集。");
                if (fetchError.name === 'AbortError') {
                    showToast('同步劇集列表超時，使用暫存數據', 'warning');
                } else {
                    showToast('同步劇集列表失敗，使用暫存數據', 'warning');
                }
            }
        } else if (historyItem) {
            // console.log(`历史记录项 "${title}" 缺少 vod_id 或 sourceName，无法刷新剧集列表。将使用已存储的剧集。`);
            showToast('無法同步劇集列表，使用暫存數據', 'info');
        }
...
function clearViewingHistory() {
    try {
        ...
        showToast('觀看紀錄已清空', 'success');
    } catch (e) {
        // console.error('清除观看历史失败:', e);
        showToast('清除觀看紀錄失敗', 'error');
    }
}
...
function clearLocalStorage() {
    ...
    modal.innerHTML = `
        <div class="bg-[#191919] rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto relative">
            ...
            <div class="mb-0">
                <div class="text-sm font-medium text-gray-300">確定要清除頁面暫存嗎？</div>
                <div class="text-sm font-medium text-gray-300 mb-4">此功能會删除你的觀看紀錄、自定義 API 接口和 Cookie，<scan class="text-red-500 font-bold">此操作不可恢复！</scan></div>
    ...
    document.getElementById('confirmBoxModal').addEventListener('click', function () {
        ...
        modal.innerHTML = `
            <div class="bg-[#191919] rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto relative">
                ...
                <div class="mb-4">
                    <div class="text-sm font-medium text-gray-300 mb-4">頁面暫存和Cookie已清除，<span id="countdown">3</span> 秒後自動刷新本頁面。</div>
                </div>
            </div>`;
